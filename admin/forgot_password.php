...
if($error!=""){
  ...
  if(count($admin)>0){
    $id_reset_password = $admin['id'];
    $email_reset_password = $admin['email'];
    
    $update_data = array(
      'password' => $pwd,
    );
    $update = executeUpdate('admin', $update_data, array('id' => $id_reset_password));     
    if($update){
      $message = "Hi ".$admin['name'].",<br>Password Reset successfully!!<br>Your login credentials are: <br> Email id: ".$email."<br> Password: ".$pwd."";
